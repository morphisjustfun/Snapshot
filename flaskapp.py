...
def loginx():
    ...
    if key in session and session[key] == [username,password]:
        return redirect('static/html/princ.html')
    db_session = db.getSession(engine)
    ...
    if len(users) > 0:
        ...
        return redirect('static/html/princ.html')
    db_session.close()
    return redirect('static/html/failed.html')
...
def authenticate():
    ...
    r_msg = {'msg':'No esta en la base de datos'}
...
def current():
    ...
    try:
        ...
        ga = session['idk']
    response = db_session.query(entities.User).filter(entities.User.id==ga)
    users = response[:]
...
def create_usersx():
    ...
    user = entities.User(username = username, name = name, fullname = fullname, password = password,contact=contact)
    db_session =  db.getSession(engine)
    db_session.add(user)
    db_session.commit()
    message = {'msg': 'User created'}
...
#@app.route('/js/<content>',methods=['GET'])
#def reda(content):
 #   link = 'js/' + content
  #  return render_template(link)

#@app.route('/css/<content>',methods=['GET'])
#def redax(content):
#    link = 'css/' + content
#    return render_template(link)

#@app.route('/images/<content>',methods=['GET'])
#def redxa(content):
#    link = 'images/' + content
#    return render_template(link)

#@app.route('/fonts/<content>',methods=['GET'])
#def redxla(content):
#    link = 'fonts/' + content
#    return render_template(link)

#@app.route('/sass/<content>',methods=['GET'])
#def redlaasa(content):
#    link = 'sass/' + content
#    return render_template(link)
